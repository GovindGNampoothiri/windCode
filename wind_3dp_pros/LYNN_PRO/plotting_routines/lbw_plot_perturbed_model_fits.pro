...
;  EXAMPLES:    
;               [calling sequence]
;               defaults = lbw_get_defaults_pmf_plots([_EXTRA=extra])
;
;  KEYWORDS:    
...
;               test = lbw_plot_perturbed_model_fits(x, param, dparm [,FUNC=func] $
;                                                    [,XLOG=xlog] [,YLOG=ylog] $
;                                                    [,_EXTRA=ex_str])
;
;               x_low = [1d-2,1d4,1d14]
...
;                   get_fitline_and_grad_multifunc.pro
;                                                                   [08/07/2015   v1.1.0]
;             8)  Forgot to update called routines (i.e., only updated in Man. pages)
;                                                                   [10/03/2016   v1.2.0]
;
;   NOTES:      
...
;   CREATED:  07/27/2015
;   CREATED BY:  Lynn B. Wilson III
;    LAST MODIFIED:  10/03/2016   v1.2.0
;    MODIFIED BY: Lynn B. Wilson III
;
;*****************************************************************************************
;-
...
;;  Let IDL know that the following are functions
FORWARD_FUNCTION lbw_get_dxdy_ranges_from_fits, lbw_get_defaults_pmf_plots, $
                 is_a_number
;;----------------------------------------------------------------------------------------
;;----------------------------------------------------------------------------------------
;;  Define some constants, dummy variables, and defaults
...
;;----------------------------------------------------------------------------------------
;;----------------------------------------------------------------------------------------
temp           = lbw_get_dxdy_ranges_from_fits(x,param,dparm,FUNC=func,XLOG=xlog, $
                                               YLOG=ylog)

test           = (SIZE(temp,/TYPE) NE 8)
IF (test[0]) THEN RETURN,0b          ;;  Error handling done within lbw_get_dxdy_ranges_from_fits.pro
;;----------------------------------------------------------------------------------------
;;  Define parameters
;;----------------------------------------------------------------------------------------
dxdy_struc     = temp
...
;;  Get defaults
defaults       = lbw_get_defaults_pmf_plots(_EXTRA=ex_str)
;;----------------------------------------------------------------------------------------
;;  Define parameters
;;----------------------------------------------------------------------------------------
