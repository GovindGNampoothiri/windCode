...
;                   get_only_good_cfa_val.pro
;                                                                   [10/23/2015   v1.0.1]
;             8)  Fixed a bug that only affected the GEN_INFO.RH_METHOD output so it
;                   now only returns the array indices limited by the use of the
;                   keywords CFA_METH_ONLY and/or FINDBEST_METH (prior versions would
;                   have, say, 200 elements in all arrays except this one, which had
;                   403 elements)
;                                                                   [11/03/2016   v1.0.2]
;
;   NOTES:      
...
;   CREATED:  02/18/2015
;   CREATED BY:  Lynn B. Wilson III
;    LAST MODIFIED:  11/03/2016   v1.0.2
;    MODIFIED BY: Lynn B. Wilson III
;
;*****************************************************************************************
;-
...
;      new_val = new__meth      ;;  Already defined these --> no need to do anything else
      old_val = new__meth      ;;  Already defined these --> now just limit if FINDBEST_METH set
      new_tag = tag_0th[zero]+'.'+tag_1st[first]   ;;  e.g., 'GEN_INFO.RH_METHOD'
      bins1   = gind_3d
      new_val = get_only_good_cfa_val(old_val,bins1)
