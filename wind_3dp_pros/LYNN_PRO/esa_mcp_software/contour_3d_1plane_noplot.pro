...
;                               a new coordinate basis onto which the data within DAT
;                               will be projected and triangulated.  The new basis will
;                               have the following:
...
;  EXAMPLES:    
;               [calling sequence]
;               contour_3d_1plane, dat, vector1, vector2 [,VLIM=vlim] [,NGRID=ngrid]   $
;                                  [,XNAME=xname] [,YNAME=yname] [,SM_CUTS=sm_cuts]    $
;                                  [,NSMOOTH=nsmooth] [,ONE_C=one_c] [,DFRA=dfra]      $
;                                  [,VCIRC=vcirc] [,EX_VEC0=ex_vec0] [,EX_VN0=ex_vn0]  $
;                                  [,EX_VEC1=ex_vec1] [,EX_VN1=ex_vn1] [,/NOKILL_PH]   $
;                                  [,/NO_REDF] [,PLANE=plane] [,/NO_TRANS]             $
;                                  [,INTERP=interp] [,SM_CONT=sm_cont] [,DFMIN=dfmin]  $
;                                  [,DFMAX=dfmax] [,DFSTR_OUT=dfstr_out]
;
;               ;;....................................................................
...
;               !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
;               ***  [Keywords passed to rotate_3dp_structure.pro]  ***
;               !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
;               C_LOG      :  If set, routine will compute and plot VDF in logarithmic
;                               instead of linear space (good for sparse data).
;                               [Default = FALSE]
;               TGRID      :  Scalar [numeric] defining the number of grid points in each
...
;                               [Default = FALSE]
;
;   CHANGED:  1)  Fixed axis label typo for PLANE 'xz'
;                                                                   [02/10/2012   v1.0.1]
;             2)  Now removes energies < 1.3 * (SC Pot) and
;                   Added keywords:  INTERP and SM_CONT
;                                                                   [02/22/2012   v1.1.0]
;             3)  Changed maximum level of contours allowed
;                                                                   [03/14/2012   v1.1.1]
...
;                                                                   [03/25/2015   v1.7.0]
;            15)  Cleaned up and added keywords: C_LOG, TGRID, and SLICE2D
;                   and now calls is_a_3_vector.pro, is_a_number.pro, and
;                   find_1d_cuts_2d_dist.pro
;                                                                   [05/27/2017   v1.8.0]
;            16)  Fixed an issue with the C_LOG keyword setting
;                                                                   [05/30/2017   v1.8.1]
;            
;             17) Added input parameter govind_n to get time series data  [08/30/2017 V1.8.1]  -By abhishek
;
;   NOTES:      
;               1)  Input structure, DAT, must have UNITS_NAME = 'Counts'
;               2)  see also:  eh_cont3d.pro, df_contours_3d.pro,
;                              df_htr_contours_3d.pro, and general_vdf_contour_plot.pro
;
;  REFERENCES:  
;               NA
;
...
;    LAST MODIFIED:  05/30/2017   v1.8.1
;    MODIFIED BY: Lynn B. Wilson III
;
;*****************************************************************************************
;-

PRO contour_3d_1plane_noplot,govind_n,dat,vector1,vector2,VLIM=vlim,NGRID=ngrid,XNAME=xname,YNAME=yname, $
                          ...
                          C_LOG=c_log,TGRID=tgrid,SLICE2D=slice2d
...
OPENW, f1,'/home/govind/data/wind/code/time.txt',/GET_LUN,/APPEND
printf,f1,time_string(data.TIME)
FREE_LUN,f1
...
IF (test[0]) THEN noredf = 0       ELSE noredf = no_redf[0]
;;  Check PLANE
test           = (N_ELEMENTS(plane) EQ 0) OR (SIZE(plane,/TYPE) NE 7)
IF (test[0]) THEN projxy = 'xy'    ELSE projxy = STRLOWCASE(plane[0])
;;  Check SM_CONT
test           = (N_ELEMENTS(sm_cont) EQ 0) OR ~KEYWORD_SET(sm_cont)
IF (test[0]) THEN sm_con = 0       ELSE sm_con = 1
;;  Check NSMOOTH
test           = (N_ELEMENTS(nsmooth) EQ 0) OR (is_a_number(nsmooth,/NOMSSG) EQ 0)
IF (test[0]) THEN ns     = 3       ELSE ns     = LONG(nsmooth)
;;  Check VECTOR1
test           = (is_a_3_vector(vector1,V_OUT=vec1,/NOMSSG) EQ 0) OR (N_ELEMENTS(vector1) LT 3)
...
ENDELSE
;;  Check VECTOR2
test           = (is_a_3_vector(vector2,V_OUT=vec2,/NOMSSG) EQ 0) OR (N_ELEMENTS(vector2) LT 3)
IF (test[0]) THEN BEGIN
...
IF (test[0]) THEN ngrid = 30L ELSE ngrid = LONG(ngrid[0])
;;  Check VLIM
...
;;  Check TGRID
test           = (N_ELEMENTS(tgrid) EQ 0) OR (is_a_number(tgrid,/NOMSSG) EQ 0)
IF (test[0]) THEN ntg = 101L ELSE ntg = (LONG(tgrid[0]) > 30L) < 300L
;;  Check SLICE2D
...
;;----------------------------------------------------------------------------------------
rotate_3dp_structure,data,vec1,vec2,VLIM=vlim,C_LOG=clog_on[0],TGRID=ntg[0],SLICE2D=slice_on[0]
;;  Define B-field at start of DF
magf_st        = vec1
IF (SIZE(onec,/TYPE) EQ 8) THEN BEGIN
  rotate_3dp_structure,onec,vec1,vec2,VLIM=vlim,C_LOG=clog_on[0],TGRID=ntg[0],SLICE2D=slice_on[0]
ENDIF
;;----------------------------------------------------------------------------------------
...
IF (test[0]) THEN evec0 = REPLICATE(f,3) ELSE evec0 = FLOAT(REFORM(ex_vec0))
test           = (N_ELEMENTS(ex_vec1) NE 3) OR ~KEYWORD_SET(ex_vec1)
IF (test[0]) THEN evec1 = REPLICATE(f,3) ELSE evec1 = FLOAT(REFORM(ex_vec1))
...
IF (test[0]) THEN vec0n = 'Vec 1' ELSE vec0n = ex_vn0[0]
test           = (N_ELEMENTS(ex_vn1) EQ 0) OR (SIZE(ex_vn1,/TYPE) NE 7)
IF (test[0]) THEN vec1n = 'Vec 2' ELSE vec1n = ex_vn1[0]
...
;  test_df = FINITE(df2d)
  lower   = ALOG10(lower_lim[0])
  upper   = ALOG10(upper_lim[0])
ENDIF ELSE BEGIN
...
IF (gd GT 0) THEN BEGIN
  mndf  = MIN(ABS(df2d[good]),/NAN) > lower[0]
  mxdf  = MAX(ABS(df2d[good]),/NAN) < upper[0]
ENDIF ELSE BEGIN
  IF (gd2 GT 0) THEN BEGIN
    ;;  some finite data
    mndf  = MIN(ABS(df2d[good2]),/NAN) > lower[0]
    mxdf  = MAX(ABS(df2d[good2]),/NAN) < upper[0]
  ENDIF ELSE BEGIN
    ;;  no finite data
...
IF KEYWORD_SET(noredf) THEN BEGIN
  ...
  dfpara         = cuts_struc.X_1D_FXY
  dfperp         = cuts_struc.Y_1D_FXY
  vpara_cut      = cuts_struc.X_CUT_COORD
  vperp_cut      = cuts_struc.Y_CUT_COORD
;  dfpara         = REFORM(df2d[*,ndf[0]])                                  ; => Para. Cut of DF
;  dfperp         = REFORM(df2d[ndf[0],*])                                  ; => Perp. Cut of DF
  IF (SIZE(onec,/TYPE) EQ 8) THEN BEGIN
    ;;  Calculate one-count parallel Cut
    cut1_struc     = find_1d_cuts_2d_dist(df1c,vx2d,vy2d,X_0=0d0,Y_0=0d0)
    onec_para      = cut1_struc.X_1D_FXY
    vpar1_cut      = cut1_struc.X_CUT_COORD
  ENDIF ELSE BEGIN
    onec_para      = REPLICATE(d,N_ELEMENTS(dfpara))
    vpar1_cut      = vx1c
  ENDELSE
ENDIF ELSE BEGIN
  ;;  Quasi-Reduced DFs
  ...
  IF (clog_on[0]) THEN good = WHERE(test_f0,gd) ELSE good = WHERE(test_f0 AND test_f1,gd)
;  good    = WHERE(test_f0 AND test_f1,gd)
  ...
  ENDIF ELSE BEGIN
    mxdf    = (mxdf[0] > MAX(ABS([dfpara[good],dfperp[good]]),/NAN)) < upper[0]
    mndf    = (mndf[0] < MIN(ABS([dfpara[good],dfperp[good]]),/NAN)) > lower[0]
    IF (SIZE(onec,/TYPE) EQ 8) THEN BEGIN
...
;;----------------------------------------------------------------------------------------
;;  Check DFMIN
test_dfmin     = (N_ELEMENTS(dfmin) NE 2) OR (is_a_number(dfmin,/NOMSSG) EQ 0)
IF (test_dfmin[0]) THEN def_low = lower[0] ELSE def_low = dfmin[0]
IF (clog_on[0]) THEN IF (~test_dfmin[0]) THEN def_low = ALOG10(def_low[0])
;;  Check DFMAX
test_dfmax     = (N_ELEMENTS(dfmax) NE 2) OR (is_a_number(dfmax,/NOMSSG) EQ 0)
IF (test_dfmax[0]) THEN def_upp = upper[0] ELSE def_upp = dfmax[0]
IF (clog_on[0]) THEN IF (~test_dfmax[0]) THEN def_upp = ALOG10(def_upp[0])
;;  Check DFRA
test           = (N_ELEMENTS(dfra) NE 2) OR (is_a_number(dfra,/NOMSSG) EQ 0)
IF (test[0]) THEN BEGIN
  df_ran  = [mndf[0],mxdf[0]]*[0.95,1.05]
ENDIF ELSE BEGIN
  sp      = SORT(dfra)
  IF (clog_on[0]) THEN df_ran = ALOG10(dfra[sp]) ELSE df_ran = dfra[sp]
ENDELSE
;;  Make sure range is not outside allowed bounds
IF (~test_dfmin[0]) THEN df_ran[0] = df_ran[0] > def_low[0]
IF (~test_dfmax[0]) THEN df_ran[1] = df_ran[1] < def_upp[0]
;;  Define contour levels
dfran    = df_ran
IF (clog_on[0]) THEN range = dfran ELSE range = ALOG10(dfran) ;;  Make levels evenly spaced in log base-10 space
;range    = ALOG10(df_ran)
lg_levs  = DINDGEN(ngrid)*(range[1] - range[0])/(ngrid - 1L) + range[0]
IF (clog_on[0]) THEN levels = lg_levs ELSE levels = 1d1^lg_levs ;;  contour level values in phase space density [e.g., s^(+3) cm^(-3) km^(-3)]
;levels   = 1d1^lg_levs
...
lim_cn0  = {XRANGE:xyran,XSTYLE:1,XLOG:0,XTITLE:xttl00,XMINOR:10, $
            ...
            POSITION:pos_0con,TITLE:con_ttl,NODATA:1,XCHARSIZE:0.7,YCHARSIZE:0.7}
...
ENDIF ELSE BEGIN
  ...
  ymin_cut = 9
ENDELSE
;;  Define cuts structure
goodyl   = WHERE(t_ytvs LE df_ran[1] AND t_ytvs GE df_ran[0],gdyl)
IF (gdyl LT 20 AND gdyl GT 0) THEN BEGIN
  ...
  lim_ct0  = {XRANGE:xyran, XSTYLE:1,XLOG:0,          XTITLE:cut_xttl,XMINOR:10,           $
              YRANGE:dfran,YSTYLE:1,YLOG:ylog_cut[0],YTITLE:yttlct,  YMINOR:ymin_cut[0],   $
              POSITION:pos_0cut,TITLE:' ',NODATA:1,YTICKS:gdyl[0]-1L,                      $
              YTICKV:t_ytvs[goodyl],YTICKNAME:ytns[goodyl],XCHARSIZE:1,YCHARSIZE:1}
ENDIF ELSE BEGIN
  ;;  structure for cuts plot without tick mark labels set
  lim_ct0  = {XRANGE:xyran, XSTYLE:1,XLOG:0,          XTITLE:cut_xttl,XMINOR:10,           $
              YRANGE:dfran,YSTYLE:1,YLOG:ylog_cut[0],YTITLE:yttlct,  YMINOR:ymin_cut[0],   $
              POSITION:pos_0cut,TITLE:' ',NODATA:1,XCHARSIZE:1,YCHARSIZE:1}
ENDELSE
;;  Define suffix for version number
...
;PLOT,[0.0,1.0],[0.0,1.0],_EXTRA=lim_ct0
  ...
  ;OPLOT,vpara_cut*1d-3,dfpars,COLOR=red[0], PSYM=4  ;; diamonds
  ;OPLOT,vperp_cut*1d-3,dfpers,COLOR=blue[0],PSYM=5  ;; triangles
;  OPLOT,vx2d*1d-3,dfpars,COLOR=red[0], PSYM=4  ;; diamonds
;  OPLOT,vy2d*1d-3,dfpers,COLOR=blue[0],PSYM=5  ;; triangles
  ;;  Plot lines
;  OPLOT,vpara_cut*1d-3,dfpars,COLOR=red[0], LINESTYLE=0
;  OPLOT,vperp_cut*1d-3,dfpers,COLOR=blue[0],LINESTYLE=0
...
;  XYOUTS,xyposi[0],xyposi[1],': Parallel Cut',/DATA,COLOR=red[0]
  ;;  Shift in negative Y-Direction
;  xyposi[1] *= 0.5
  ;;  Output label for perp
;  XYOUTS,xyposi[0],xyposi[1],': Perpendicular Cut',COLOR=blue[0],/DATA
  ;;--------------------------------------------------------------------------------------
  ;;  Plot 1-Count Level if desired
  ;;--------------------------------------------------------------------------------------
;  IF (SIZE(onec,/TYPE) EQ 8) THEN BEGIN
;   OPLOT,vpar1_cut*1d-3,onec_par,COLOR=150,LINESTYLE=4
...
;;----------------------------------------------------------------------------------------
; XYOUTS,0.795,0.06,version[0]+vers_suffx[0],CHARSIZE=.65,/NORMAL,ORIENTATION=90.
;;----------------------------------------------------------------------------------------
