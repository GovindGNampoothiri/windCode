...
;                                                                   [02/19/2016   v3.1.1]
;            14)  Addressed an issue with the use of the RM_PHOTO_E keyword
;                                                                   [01/02/2017   v3.1.2]
;
;   NOTES:      
...
;    LAST MODIFIED:  01/02/2017   v3.1.2
...
;  charge = define_particle_charge(data,E_SHIFT=e_shift)
  charge = define_particle_charge(data3d,E_SHIFT=e_shift)
  ...
  oldener = data3d[0].ENERGY
  temp    = (oldener + e_shift[0]) + (scpot[0]*charge[0])
;  temp    = (data3d[0].ENERGY + e_shift[0]) + (scpot[0]*charge[0])
;  temp   = data3d[0].ENERGY + e_shift[0]
;  temp  += (scpot[0]*charge[0])
  bad     = WHERE(temp LE 0e0 OR oldener LE 1.3*scpot[0],bd)
