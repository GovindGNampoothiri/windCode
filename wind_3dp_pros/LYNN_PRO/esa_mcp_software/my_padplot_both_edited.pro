...
PRO my_padplot_both_edited,tempdat,NOCOLOR=nocolor,LIMITS=limits,UNITS=units,COLOR=shades, $
...
energy_val=[1113.0,689.2,426.8,264.8,165.0,103.3,65.2,41.8,27.2,27.2,27.2,27.2,27.2]

conv_factor=2e5/((5.6e-6)^2)*energy_val
...
IF (FINITE(ymin) AND FINITE(ymax) AND ymin GT 0. AND ymax GT 0.) THEN BEGIN
  plotlim = {TITLE:title,XMINOR:9,XTICKNAME:['0','45','90','135','180'], $
             ...
             YTITLE:ytitle,YMARGIN:[3,5],YLOG:1,YRANGE:[ymin*conv_factor[0],ymax*conv_factor[8]]        }
ENDIF ELSE BEGIN  
  plotlim = {TITLE:title,XMINOR:9,XTICKNAME:['0','45','90','135','180'], $
             XTITLE:xtitle,XMARGIN:xmarg,XSTYLE:1,XRANGE:[0.,180.],      $
             XTICKV:[0.,45.,90.,135.,180.], XTICKS:4,YMINOR:9,YSTYLE:1,  $
...
PLOT,xdat[*,0],ydat[*,0]*conv_factor[0],/NODATA,_EXTRA=plotlim
  ...
  FOR j=0L, diffen-1L DO BEGIN
    ...
    OPLOT,xdat[*,i],ydat[*,i]*conv_factor[i],COLOR=my_colors[j]
    OPLOT,xdat[*,i],ydat[*,i]*conv_factor[i],COLOR=my_colors[j],psym=2,symsize=0.8
