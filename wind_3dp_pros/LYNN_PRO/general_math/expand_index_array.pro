...
;                                                                   [07/21/2016   v1.0.0]
;             4)  Fixed an issue when NO_WRAP=0
;                                                                   [02/05/2017   v1.0.1]
;
;   NOTES:      
...
;    LAST MODIFIED:  02/05/2017   v1.0.1
...
;  STOP
  IF (bd_low[0] GT 0) THEN BEGIN
    temp             = arrout[bad_low]
    diff             = temp - lower[0]
    arrout[bad_low]  = upper[0] + diff + d_arout[0]
;    arrout[bad_low]  = upper[0] + REVERSE(diff) + d_arout[0]
  ENDIF
;  IF (bd_low[0] GT 0) THEN arrout[bad_low] += n__in[0]
;  IF (bd_upp[0] GT 0) THEN arrout[bad_upp]  = (arrout[bad_upp] MOD upper[0]) - 1L
  IF (bd_upp[0] GT 0) THEN BEGIN
;    arrout[bad_upp]  = ((arrout[bad_upp] MOD upper[0]) - 1L) + lower[0]
    arrout[bad_upp]  = (arrout[bad_upp] - upper[0] + lower[0]) -  d_arout[0]
  ENDIF
