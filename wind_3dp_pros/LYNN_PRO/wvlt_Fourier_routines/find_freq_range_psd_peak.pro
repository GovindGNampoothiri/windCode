...
;  EXAMPLES:    
;               [calling sequence]
;               fran_peak = find_freq_range_psd_peak(time, field [,FREQRA=freqra]        $
;                                                    [,FPKMIN=fpkmin] [,FPKMAX=fpkmax]   $
;                                                    [,ALPHA=alpha] [,/SYMMETRY]         $
;                                                    [,SYMM_THRESH=symm_thresh]          $
;                                                    [,LENGTH=length] [,OFFSET=offset]   $
;                                                    [,OUTPUT=wav_struc]                 )
;
;               ;;  Find spectral peak frequency range between 1 Hz and 10 kHz
;               fran_peak = find_freq_range_psd_peak(time,field,FREQRA=[1d0,1d4])
;
;  KEYWORDS:    
;               **********************************
;               ***       DIRECT  INPUTS       ***
...
;                                                                   [04/28/2015   v1.1.0]
;             9)  Updated Man. page and now allows OFFSET to be as small as
;                   LENGTH/2
;                                                                   [06/03/2016   v1.2.0]
;
;   NOTES:      
...
;               2)  Routine constrains the following:  OFFSET ≥ MAX([LENGTH/2,2])
;
;  REFERENCES:  
...
;;    --> LBW III  06/03/2016   v1.2.0
;nshft          = nshft[0] > nlen[0]       ;;  Make sure OFFSET is at least LENGTH
nshft          = nshft[0] > ((nlen[0]/2L) > 2L)  ;;  Make sure OFFSET is at least LENGTH/2
;;----------------------------------------------------------------------------------------
...
tempx          = TOTAL(REFORM(envelope_x[*,*,0]),1,/NAN,/DOUBLE)
tempf          = TOTAL(FINITE(REFORM(envelope_x[*,*,0])),1,/NAN,/DOUBLE)
env_xx         = tempx/tempf        ;;  Avg. frequencies [Hz] of binned range
tempy          = REFORM(envelope_x[*,*,1])
tempyl         = MIN(tempy,/NAN,DIMENSION=1)
tempyu         = MAX(tempy,/NAN,DIMENSION=1)
env_yy[*,0]    = tempyl             ;;  Lower bound on PSD [units^2 Hz^(-1)] in binned range
env_yy[*,1]    = tempyu             ;;  Upper bound on PSD [units^2 Hz^(-1)] in binned range
;;    --> LBW III  06/03/2016   v1.2.0
