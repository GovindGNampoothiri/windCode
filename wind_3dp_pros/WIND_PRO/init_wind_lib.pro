...
;               2)  The following shared object libraries:
;                     wind_lib.so
;                     wind3dp_lib_darwin_ppc.so
;                     wind3dp_lib_darwin_i386.so
;                     wind3dp_lib_darwin_x86_64.so
;                     wind3dp_lib_sunos_sparc.so
;                     wind3dp_lib_sunos_unix_x86_64.so
;                     wind3dp_lib_ss32.so
;                     wind3dp_lib_ls32.so
;                     wind3dp_lib_linux_x86.so
;                     wind3dp_lib_linux_x86_64.so
;
;  INPUT:
;               NA
;
...
;                                                                   [09/30/2015   v1.4.0]
;            10)  Fixed some error handling associated with 64-bit architecture
;                                                                   [07/19/2016   v1.4.1]
;            11)  Updated to handle 64-bit Solaris with 'x86_64' ARCH and 'unix'
;                   OS_FAMILY
;                                                                   [08/09/2016   v1.4.2]
;            12)  Forgot to allow for the new shared object library when checking for
;                   64-bit architectures
;                                                                   [08/09/2016   v1.4.3]
;
;   NOTES:      
;               1)  The shared object libraries listed above are pre-compiled
;                     decomutation software that is operating system (OS) dependent,
;                     which is why there are so many of them.  These libraries are
;                     required to read the level zero (lz) 3DP binary files.
;               2)  Please see help_3dp.html for information on creating the master file
;                     for 3DP level zero data allocation.
;
;  REFERENCES:  
;               0)  Lin et al., (1995), "A Three-Dimensional Plasma and Energetic
;                     particle investigation for the Wind spacecraft," Space Sci. Rev.
;                     Vol. 71, pp. 125.
;                     http://adsabs.harvard.edu/abs/1995SSRv...71..125L
;               1)  LZ data files found at:
...
;    LAST MODIFIED:  08/09/2016   v1.4.3
...
sun_sofs       = ['wind3dp_lib_sunos_sparc.so','wind3dp_lib_sunos_unix_x86_64.so']
...
oskern         = STRLOWCASE(!VERSION.OS)         ;;  e.g., 'darwin'
...
IF (vrel[0] GE '5.4') THEN BEGIN
  ;;  Using an IDL version newer than v5.4 --> make sure .so file exists
  testb  = (mbits[0] NE 32)                                      ;;  TRUE  :  64-bit IDL being used
  testos = (osname[0] NE 'linux') AND (osname[0] NE 'solaris')   ;;  TRUE  :  not allowed OS_NAME values
  testkn = (oskern[0] NE 'darwin')                               ;;  TRUE  :  not allowed OS kernel
  test  = testb[0] AND (testos[0] AND testkn[0])
;  test  = (mbits[0] NE 32) AND ((osname[0] NE 'linux') AND (oskern[0] NE 'darwin'))
;  test  = (mbits[0] NE 32) AND (osname[0] NE 'linux')
  IF (test[0]) THEN BEGIN
    ;;  TRUE  :  No currently available shared object library for this config. --> exit
    wind_lib = 0
...
;    ENDELSE
  ...
  'mac os x' : BEGIN                  ;;  Mac OS X Machine
    CASE arch[0] OF
      'i386'    :  libname = mac_sofs[1]   ;;  Intel chip [32-bit]
      'x86_64'  :  libname = mac_sofs[2]   ;;  Intel chip [64-bit]
      ELSE      :  libname = mac_sofs[0]   ;;  PowerPC chip [32-bit]
    ENDCASE
;    IF (arch[0] EQ 'i386') THEN libname = mac_sofs[1] ELSE libname = mac_sofs[0]
  END
  ELSE       : BEGIN
    ;;  Unix Machine
    libname = unx_sofs[(vrel[0] GE '5.5')]
